...
public class BST<Key extends Comparable<Key>, Value> {
	...
	public Key select (Node parent, int k) {
		if (parent == null) { return null; }
		
		// how many in left-tree (including 0 if empty)
		int numLeft = size(parent.left);
		
		// if there are more nodes in left sub-tree than k, we can safely recurse there.
		if (numLeft > k) { return select (parent.left, k); }
		
		// we need to go to right sub-tree, but adjust by subtract numLeft as well as 1 for self.
		if (numLeft < k) { return select (parent.right, k - numLeft - 1); }
