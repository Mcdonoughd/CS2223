package dmcdonough.hw4.expr;

import java.util.Stack;   // for convenience, continue to use existing JDK Stack class...

import dmcdonough.hw4.expr.nodes.NoParameterOperatorNode;
import dmcdonough.hw4.expr.nodes.BinaryOperatorNode;
import dmcdonough.hw4.expr.nodes.DivisionNode;
import dmcdonough.hw4.expr.nodes.ENode;
import dmcdonough.hw4.expr.nodes.ExpressionNode;
import dmcdonough.hw4.expr.nodes.MinusNode;
import dmcdonough.hw4.expr.nodes.NumberNode;
import dmcdonough.hw4.expr.nodes.OperatorNode;
import dmcdonough.hw4.expr.nodes.PiNode;
import dmcdonough.hw4.expr.nodes.PlusNode;
import dmcdonough.hw4.expr.nodes.PowNode;
import dmcdonough.hw4.expr.nodes.SqrtNode;
import dmcdonough.hw4.expr.nodes.TimesNode;
import dmcdonough.hw4.expr.nodes.TriNode;
import dmcdonough.hw4.expr.nodes.UnaryOperatorNode;
...
public class Evaluate {
	public static void main(String[] args) {
		...
		while (!StdIn.isEmpty()) {
			...
			else if (s.equals ("-")) { ops.push(new MinusNode(s)); }
			else if (s.equals ("*")) { ops.push(new TimesNode(s)); }
			else if (s.equals ("/")) { ops.push(new DivisionNode(s)); }
			else if (s.equals ("sqrt")) { ops.push(new SqrtNode(s)); }
			else if (s.equals ("triangle")) { ops.push(new TriNode(s));}
			else if (s.equals("^")) { ops.push(new PowNode(s));}
